...
protected:
	...
	bool add_event_handler(event_handler_t h_);
...
private:
	...
	// private variables
	float frame_time;
	std::map<std::type_index, event_handler_t> handlers;
...
template<typename EventType>
bool Game_base::add_event_handler(event_handler_t h_)
{
    handlers[std::type_index(typeid(EventType))] = h_;
    return true;
}
