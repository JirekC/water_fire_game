...
{
	const auto* kp = ev_.getIf<sf::Event::KeyPressed>();
	if (!kp) return;
	for (int i = 0; i < (int)key_shortcuts.size(); i++)
	{
		if (kp->code == key_shortcuts[i])
		{
...
{
	const auto* kr = ev_.getIf<sf::Event::KeyReleased>();
	if (!kr) return;
	for (int i = 0; i < (int)key_shortcuts.size(); i++)
	{
		if (kr->code == key_shortcuts[i])
		{
